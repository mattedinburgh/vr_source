...
function Get-LobotRegisteredNames {
    param(
        [Parameter(Mandatory=$true)][string]$Source,
        [Parameter(Mandatory=$true)][string]$Enumerator
    )

    $pattern = '(?s)LOGBT_ENUMDB_ADD\("' + [regex]::Escape($Enumerator) + '"\s*,.*?\);'
    $match = [regex]::Match($Source, $pattern)
    if (-not $match.Success) {
        throw "Could not find LOBOT enumerator registration: $Enumerator"
    }

    $block = [regex]::Replace($match.Value, '(?s)/\*.*?\*/', '')
...
, '')
    ...
    foreach ($token in [regex]::Matches($block, '\b[A-Z][A-Z0-9_]+\b')) {
        $name = $token.Value
        if ($name -eq "LOGBT_ENUMDB_ADD" -or $name -like "NUM*") { continue }
        [void]$set.Add($name)
    }
    return ,$set
}
...
function Expand-VengeanceEquipmentLayers {
    param(
        ...
        [Parameter(Mandatory=$true)]$ValidAnimSurfaces,
        [Parameter(Mandatory=$true)]$ValidAnimStates
    )

    $layersToAdd = @("legarmor","facegear","gasmask","ears","backpack","legrig","legrig_left","knees")
    $temps = New-Object System.Collections.ArrayList
    try {
        $gearTemp = Join-Path $env:TEMP ("vr_lobot_" + $Key + "_gear_" + $PID + ".xml")
        [void]$temps.Add($gearTemp)
        ...
        foreach ($layerName in $layersToAdd) {
            if ($targetDoc.SelectSingleNode("/LogicalAnimationSurfaces/Layer[@name='$layerName']")) {
                continue
            }
            ...
            foreach ($surface in @($clone.SelectNodes(".//Surface"))) {
                ...
                if ($surface.HasAttribute("animsurface")) {
                    $valid = $ValidAnimSurfaces.Contains($surface.GetAttribute("animsurface"))
                }
                elseif ($surface.HasAttribute("animstate")) {
                    $valid = $ValidAnimStates.Contains($surface.GetAttribute("animstate"))
                }
        ...
        foreach ($catalogRelative in $CatalogRelatives) {
            $catalogTemp = Join-Path $env:TEMP ("vr_lobot_" + $Key + "_catalog_" + [Math]::Abs($catalogRelative.GetHashCode()) + "_" + $PID + ".xml")
...
$bodyTypeDbPath = Join-Path $ScriptRoot "Tactical\LogicalBodyTypes\BodyTypeDB.cpp"
if (-not (Test-Path $bodyTypeDbPath)) {
    throw "Cannot validate Vengeance animation enumerators: $bodyTypeDbPath"
}
$bodyTypeDbSource = [System.IO.File]::ReadAllText($bodyTypeDbPath)
$validAnimSurfaces = Get-LobotRegisteredNames $bodyTypeDbSource "AnimationSurfaces"
$validAnimStates = Get-LobotRegisteredNames $bodyTypeDbSource "AnimationStates"
