...
//	}
	...
	{
		if (!pTargetSoldier->CanBeHandcuffed())
			return ITEM_HANDLE_REFUSAL;
	...
	for(i=0 ; i<invsize; ++i)
	{
			 ...
			 (pOpponent->bTeam != gbPlayerNum || !(Item[pObject->usItem].defaultundroppable)) )
		{
			...
			else
			{
				...
				{
					// Vengeance: a successful steal can target the enemy's entire physical inventory.
					// Do not whitelist only hand/pocket slots: worn armour, helmets, face gear,
					// LBE, backpacks, slung weapons and every other populated inventory slot
					// are valid steal targets as well.
					fStealItem = TRUE;
				}
			}
