...
static void SpawnVRDirectionalGoreSpray( SOLDIERTYPE *pSoldier, const CHAR8 *zFilename, UINT8 ubSprayDirection, INT16 sZ, INT16 sDelay, UINT8 ubOffsetPercent )
{
	...
	ANITILE *pGoreAni = CreateAnimationTile( &AniParams );
	if ( pGoreAni != NULL && pGoreAni->pLevelNode != NULL )
	{
		// Keep blood visibly deeper/darker than the default world palette. The STI
		// colours stay intact; the engine applies two extra shade steps at render time.
		pGoreAni->pLevelNode->ubShadeLevel = (UINT8)__min( MAX_SHADE_LEVEL, DEFAULT_SHADE_LEVEL + 2 );
		pGoreAni->pLevelNode->ubNaturalShadeLevel = pGoreAni->pLevelNode->ubShadeLevel;
	}
}
...
static BOOLEAN HandleVRCinematicGunshotReaction( SOLDIERTYPE *pSoldier, UINT16 usWeaponIndex, INT16 sDamage, UINT16 bDirection, UINT8 ubHitLocation )
{
	...
	UINT8 ubSoftLeftDirection = (UINT8)( ( ubIncomingDirection + 7 ) % NUM_WORLD_DIRECTIONS );
	UINT8 ubSoftRightDirection = (UINT8)( ( ubIncomingDirection + 1 ) % NUM_WORLD_DIRECTIONS );

	BOOLEAN fRunning = ( pSoldier->usAnimState == RUNNING || pSoldier->usAnimState == RUNNING_W_PISTOL );
	UINT8 ubMomentumDirection = ubOriginalDirection;
...
static BOOLEAN HandleVRFatalGunshotReaction( SOLDIERTYPE *pSoldier, UINT16 usWeaponIndex, INT16 sDamage, UINT16 bDirection, UINT8 ubHitLocation )
{
	...
	if ( !( Item[ usWeaponIndex ].usItemClass & IC_GUN ) || pSoldier->MercInWater() )
		return FALSE;
	...
	BOOLEAN fHadMomentum = ( ( gAnimControl[ pSoldier->usAnimState ].uiFlags & ANIM_MOVING ) != 0 );
	UINT8 ubMomentumDirection = pSoldier->ubDirection;

	if ( fHadMomentum && pSoldier->pathing.usPathIndex < pSoldier->pathing.usPathDataSize )
		ubMomentumDirection = (UINT8)( pSoldier->pathing.usPathingData[ pSoldier->pathing.usPathIndex ] % NUM_WORLD_DIRECTIONS );

	UINT8 ubRagdollBaseDirection = fHadMomentum ? ubMomentumDirection : ubIncomingDirection;
	UINT8 ubRagdollLeftDirection = (UINT8)( ( ubRagdollBaseDirection + 6 ) % NUM_WORLD_DIRECTIONS );
	UINT8 ubRagdollRightDirection = (UINT8)( ( ubRagdollBaseDirection + 2 ) % NUM_WORLD_DIRECTIONS );
	...
	// Pseudo-ragdoll bias: a moving victim often carries his existing velocity into
	// the death instead of snapping to a stationary pose. Hit-location gore remains
	// independent, so this can safely override the body-motion family for testing.
	if ( fHadMomentum && Random( 100 ) < 65 )
	{
		static const UINT8 aubMomentumVariants[ 6 ] = { 0, 2, 3, 4, 27, 28 };
		ubVariant = aubMomentumVariants[ Random( 6 ) ];
	}
	...
	case 25: // catastrophic full-body dismemberment
		...
		SpawnVRDirectionalGoreSpray( pSoldier, "TILECACHE\\VR_GORE_SPRAY_MEDIUM.STI", ubExitDirection, 29, 44, 60 );
		DropVRDirectionalBloodTrail( pSoldier, ubExitDirection, 6, 6 );
	...
	case 27: // triple-layer backward blast
		...
		SpawnVRDirectionalGoreSpray( pSoldier, "TILECACHE\\VR_GORE_SPRAY_MEDIUM.STI", ubExitDirection, 28, 45, 70 );
	...
	case 28: // huge mist cone followed by forward collapse
		SpawnVRDirectionalGoreSpray( pSoldier, "TILECACHE\\VR_GORE_SPRAY_MEDIUM.STI", ubExitDirection, 39, 29, 0 );
	...
	if ( pSoldier->pathing.bDesiredDirection != pSoldier->ubDirection )
	{
		if ( (gAnimControl[ usAnimState ].uiFlags & ( ANIM_BREATH | ANIM_OK_CHARGE_AP_FOR_TURN | ANIM_FIREREADY/* | ANIM_TURNING*/ ) || usForceAnimState != INVALID_ANIMATION) && !fInitalMove && !pSoldier->flags.fDontChargeTurningAPs )//dnl ch70 160913 //dnl ch73 290913 in some bright future when UIPlotPath will calculate turning cost then ANIM_TURNING should be turn on as this solve many problems when APs was not deducted during turnoff
		{
			...
			if (HAS_SKILL_TRAIT( pSoldier, MARTIAL_ARTS_NT ) && gGameOptions.fNewTraitSystem )
				sAPCost = max(1, (INT16)(((FLOAT)sAPCost * (FLOAT)(100 - gSkillTraitValues.ubMAApsTurnAroundReduction * NUM_SKILL_TRAITS(pSoldier, MARTIAL_ARTS_NT)) / 100.0f) + 0.5f));
		...
		for ( INT8 bLoop = 0; bLoop < invsize; ++bLoop)									// ... for all items in our inventory ...
		{
			if ( this->inv[bLoop].exists() )
			{
				...
				if ( checkfurther )
				{
					...
					// sevenfm: consider suppressors, lasers and trigger groups as military hardware
...
BOOLEAN		SOLDIERTYPE::LooksLikeASoldier( BOOLEAN fShowResult )
{
	...
	for ( INT8 bLoop = 0; bLoop < invsize; ++bLoop)									// ... for all items in our inventory ...
	{
		if ( this->inv[bLoop].exists() )
		{
			...
			// sevenfm: any drugs in soldier's inventory
			/*if ( Item[this->inv[bLoop].usItem].drugtype != 0 )
			{
				if(fShowResult) ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"%s has drugs", this->GetName() );
				return FALSE;
			}*/
		}
	}
...
BOOLEAN		SOLDIERTYPE::EquipmentTooGood( BOOLEAN fCloselook, BOOLEAN fShowResult )
{
	...
	if ( UsingNewInventorySystem() )
	{
		...
		for ( INT8 bLoop = 0; bLoop < invsize; ++bLoop)									// ... for all items in our inventory ...
		{
			if ( this->inv[bLoop].exists() )
			{
				...
				// sevenfm: any drugs in soldier's inventory
				/*if ( fCloselook && Item[this->inv[bLoop].usItem].drugtype != 0 )
				{
					if(fShowResult) ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, L"%s has drugs", this->GetName() );
					return TRUE;
				}*/
...
BOOLEAN		SOLDIERTYPE::SeemsLegit( UINT8 ubObserverID, BOOLEAN fShowResult )
{
	...
	if (FindBombNearby(pSoldier, this->sGridNo, 1))
	{
	...
	{
		// always uncover civilians in restricted area
		if ( ubSectorData > 1 )
		{
		...
		// is it night?
		else if ( ubSectorData == 1 && NightLight() )
		{
	...
	if ( this->usSoldierFlagMask & SOLDIER_COVERT_SOLDIER )
	{
		...
		// always uncover drunk soldiers
		if ( fCloseLook && GetDrunkLevel( this ) != SOBER )
		{
			if(fShowResult) ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, szCovertTextStr[STR_COVERT_DRUNKEN_SOLDIER], this->GetName() );
			return FALSE;
		}
		// uncover if soldier is under drugs
		if ( fCloseLook && MercUnderTheInfluence( this ) )
		{
			if(fShowResult) ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, szCovertTextStr[STR_COVERT_DRUNKEN_SOLDIER], this->GetName() );
			return FALSE;
		}

		// only inspect soldier's equipment if he is close and observer is officer of the same type		
		if ( this->EquipmentTooGood( fCloseLook && pSoldier->CanInspect(this), fShowResult) )
		{
...
void SOLDIERTYPE::Disguise( void )
{
	...
	// cannot disguise with high suspicion level
	if( SuspicionPercent() >= 100 )
	{
		return;
	}
	...
	if ( !(this->usSoldierFlagMask & (SOLDIER_DAMAGED_VEST | SOLDIER_DAMAGED_PANTS)) )
	{
		...
		if ( !(this->usSoldierFlagMask & SOLDIER_COVERT_SOLDIER) )
		{
			...
			if( COMPARE_PALETTEREP_ID(this->VestPal, "WHITEVEST") &&
				( COMPARE_PALETTEREP_ID(this->PantsPal, "BLUEPANTS") || COMPARE_PALETTEREP_ID(this->PantsPal, "JEANPANTS")))
			{
...
UINT8 SOLDIERTYPE::SuspicionPercent( void )
{
	return __min(100, 100 * usSkillCounter[SOLDIER_COUNTER_SUSPICION] / (APBPConstants[AP_MAXIMUM] * MAX_SUSPICION));
}

void SOLDIERTYPE::AddSuspicion( UINT32 iValue )
{
	usSkillCounter[SOLDIER_COUNTER_SUSPICION] += iValue;
	usSkillCounter[SOLDIER_COUNTER_SUSPICION] = __min( usSkillCounter[SOLDIER_COUNTER_SUSPICION], APBPConstants[AP_MAXIMUM] * MAX_SUSPICION );
	usSkillCounter[SOLDIER_COUNTER_SUSPICION] = __max( usSkillCounter[SOLDIER_COUNTER_SUSPICION], 0);
}

void SOLDIERTYPE::MultiplySuspicionByPercent( UINT8 ubPercent )
{
	usSkillCounter[SOLDIER_COUNTER_SUSPICION] = usSkillCounter[SOLDIER_COUNTER_SUSPICION] * ubPercent / 100;
	usSkillCounter[SOLDIER_COUNTER_SUSPICION] = __min( usSkillCounter[SOLDIER_COUNTER_SUSPICION], APBPConstants[AP_MAXIMUM] * MAX_SUSPICION );
	usSkillCounter[SOLDIER_COUNTER_SUSPICION] = __max( usSkillCounter[SOLDIER_COUNTER_SUSPICION], 0);
}

void SOLDIERTYPE::SetMaxSuspicion( void )
{
...
BOOLEAN SOLDIERTYPE::CanInspect( SOLDIERTYPE *pOpponent )
{
	...
	// covert civilians can be inspected by any officer
	if( pOpponent->usSoldierFlagMask & SOLDIER_COVERT_CIV )
	{
		return TRUE;
	}
...
BOOLEAN TwoStagedTrait( UINT8 uiSkillTraitNumber )
{
	...
	return( uiSkillTraitNumber > 0 && (uiSkillTraitNumber <= NUM_ORIGINAL_MAJOR_TRAITS || uiSkillTraitNumber == COVERT_NT || uiSkillTraitNumber == PILOT_NT) );
}
