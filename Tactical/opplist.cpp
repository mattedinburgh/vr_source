...
INT16 DistanceVisible( SOLDIERTYPE *pSoldier, INT8 bFacingDir, INT8 bSubjectDir, INT32 sSubjectGridNo, INT8 bLevel )
{
	...
	if (UsingNewVisionSystem() &&
		...
		SpacesAway(sSubjectGridNo, pSoldier->sMTActionGridNo) > 1)
	{
		sDistVisible = min(sDistVisible, 5);
	}
	...
	// sevenfm: don't allow negative distance
	return max(sDistVisible, 0);
}
	...
	*pPersOL = SEEN_THIS_TURN;
	...
	if ( (pSoldier->ubCivilianGroup == KINGPIN_CIV_GROUP) && (pOpponent->bTeam == gbPlayerNum ) )
	{
		...
		if ( InARoom( pOpponent->sGridNo, &usRoom ) && IN_BROTHEL( usRoom ) && ( IN_BROTHEL_GUARD_ROOM( usRoom ) ) )
		{
			...
			DebugAI(AI_MSG_INFO, pSoldier, String("CancelAIAction: make guard run to block guard room"));
...
void ManSeesMan(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pOpponent, INT32 sOppGridNo, INT8 bOppLevel, UINT8 ubCaller, UINT8 ubCaller2)
{
	...
	{
		// 1.13 role discovery: a player merc observing an enemy advances identification.
		if ( pOpponent->bTeam == ENEMY_TEAM && pSoldier->bTeam == gbPlayerNum )
			...
			else if ( pSoldier->IsAssassin() && pSoldier->bTeam == CIV_TEAM )
			{
				...
				if ( fNotAddedToList )
				{
					...
					if ( pSoldier->aiData.bOrders != SEEKENEMY )
					{
						...
						{
							// didn't see anyone before!
							DebugAI(AI_MSG_INFO, pSoldier, String("CancelAIAction: assassin: didn't see anyone before!"));
							CancelAIAction( pSoldier, TRUE );
							SetNewSituation( pSoldier );
						}
					}
...
//Ja25 No meanwhiles
	...
	if(gGameExternalOptions.fShowNoiseLocator)
	{
		...
		{
			// Unseen gunfire gives only a coarse bearing. Do not flash the hidden
			// shooter or the exact source tile.
			if ( ubNoiseType == NOISE_GUNFIRE && !fNoiseMakerSeen )
			{
				...
				if ( !TileIsOutOfBounds( sBearingGridNo ) )
					BeginUnseenFireBearingCue( sBearingGridNo, pSoldier->pathing.bLevel, ubNoiseDir );
			}
