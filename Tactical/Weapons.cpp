...
// CalcNewChanceToHitGun is entered only when NCTH is active; keep one authoritative calculation path.
//////////////////////////////////////////////////////////////////////////////////
...
UINT32 CalcChanceToHitGun(SOLDIERTYPE *pSoldier, INT32 sGridNo, INT16 sAimTime, UINT8 ubAimPos )
{
	...
	// determine vision without scope to know bonus from raising weapon
	gbForceWeaponNotReady = true;
	sDistVisNoScope = pSoldier->GetMaxDistanceVisible(sGridNo, pSoldier->bTargetLevel, CALC_FROM_ALL_DIRS ) * CELL_X_SIZE;
	gbForceWeaponNotReady = false;
...
void ShotMiss( UINT8 ubAttackerID, INT32 iBullet )
{
	...
	if ( fDoMissForGun )
	{
		...
		if ( UsingNewCTHSystem() )
		{
			...
			// BattleLogAddNCTHMiss itself filters to player-involved shots.
