...
BOOLEAN DamageSoldierFromBlast( UINT8 ubPerson, UINT8 ubOwner, INT32 sBombGridNo, INT16 sWoundAmt, INT16 sBreathAmt, UINT32 uiDist, UINT16 usItem, INT16 sSubsequent , BOOL fFromRemoteClient )
{
	...
	else
	{
		if ( ubOwner < TOTAL_SOLDIERS && (MercPtrs[ ubOwner ] != NULL) && gGameOptions.fNewTraitSystem)
		{
			...
			{
				sNewWoundAmt = (INT16)((((FLOAT)sNewWoundAmt * (FLOAT)(100 + gSkillTraitValues.ubDEDamageOfBombsAndMines)) / 100.0f) + 0.5f);
			}
			...
			{
				sNewWoundAmt = (INT16)((((FLOAT)sNewWoundAmt * (FLOAT)(100 + gSkillTraitValues.ubHWDamageTanksBonusPercent * NUM_SKILL_TRAITS(MercPtrs[ubOwner], HEAVY_WEAPONS_NT))) / 100.0f) + 0.5f); // +30%
			}
			...
			{
				sNewWoundAmt = (INT16)((((FLOAT)sNewWoundAmt * (FLOAT)(100 + gSkillTraitValues.ubHWDamageBonusPercentForHW * NUM_SKILL_TRAITS(MercPtrs[ubOwner], HEAVY_WEAPONS_NT))) / 100.0f) + 0.5f); // +15%
			}
		}
...
// Flugente: remove any marker that an explosion is active but has not yet occurred (to be used when leaving a sector)
...
void FireFragments( UINT8 ubOwner, INT16 sX, INT16 sY, INT16 sZ, UINT16 usItem, UINT8 ubDirection )
{
	...
	for (UINT16 x = 0; x < usNumFragments; ++x)
	{
		...
		// sevenfm: for explosions on the ground, don't send fragments down into the ground
		if (sZ <= 0 && dEndZ < 0)
		{
			dEndZ = -dEndZ;
		}
