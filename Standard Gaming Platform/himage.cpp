...
static BOOLEAN VHDUnpackETRLERegionRGBA( HIMAGE hImage, UINT16 usIndex, std::vector<UINT8> &out )
{
	if ( hImage == NULL || hImage->pETRLEObject == NULL || hImage->pPixData8 == NULL ||
		 hImage->pPalette == NULL || usIndex >= hImage->usNumberOfObjects )
		return FALSE;

	const ETRLEObject *pRegion = &hImage->pETRLEObject[ usIndex ];
	const UINT32 uiPixelCount = (UINT32)pRegion->usWidth * (UINT32)pRegion->usHeight;
	if ( uiPixelCount > 0x3FFFFFFFu )
		return FALSE;

	out.assign( (size_t)uiPixelCount * 4, 0 );
	...
	while ( usY < pRegion->usHeight && pSrc < pEnd )
	{
		const UINT8 ubCode = *pSrc++;

		if ( ubCode == 0 )
		{
			if ( usX != pRegion->usWidth )
				return FALSE;
			++usY;
			usX = 0;
			continue;
		...
		if ( ubCode & 0x80 )
		{
			if ( (UINT32)usX + ubCount > pRegion->usWidth )
				return FALSE;
			usX = (UINT16)( usX + ubCount );
		}
		else
		{
			if ( (UINT32)usX + ubCount > pRegion->usWidth || pSrc + ubCount > pEnd )
				return FALSE;

			for ( UINT8 i = 0; i < ubCount; ++i, ++usX )
			{
				const UINT8 ubPaletteIndex = *pSrc++;
				const SGPPaletteEntry &pal = hImage->pPalette[ ubPaletteIndex ];
				UINT8 *pDst = &out[ ( ( (size_t)usY * pRegion->usWidth ) + usX ) * 4 ];
				pDst[0] = pal.peRed;
				pDst[1] = pal.peGreen;
				pDst[2] = pal.peBlue;
				pDst[3] = 255;
	...
	if ( usY == pRegion->usHeight )
		return TRUE;
	return ( usY + 1 == pRegion->usHeight && usX == pRegion->usWidth );
}

BOOLEAN ScaleIndexedImageToTrueColorForVHD( HIMAGE hImage, UINT8 ubScale )
{
	if ( hImage == NULL || ( ubScale != 2 && ubScale != 4 ) )
		return FALSE;
	if ( hImage->ubBitDepth != 8 || !( hImage->fFlags & IMAGE_BITMAPDATA ) ||
		 !( hImage->fFlags & IMAGE_TRLECOMPRESSED ) || hImage->usNumberOfObjects == 0 ||
		 hImage->pETRLEObject == NULL || hImage->pPixData8 == NULL || hImage->pPalette == NULL )
		return FALSE;
	...
	for ( UINT16 i = 0; i < usCount; ++i )
	{
		...
		const UINT32 uiPixels = uiNewWidth32 * uiNewHeight32;
		if ( uiPixels > 0x3FFFFFFFu )
			return FALSE;
		const UINT32 uiBytes = uiPixels * 4;
		if ( 0xFFFFFFFFu - uiTotalBytes < uiBytes )
			return FALSE;

		ETRLEObject &dstRegion = newRegions[i];
		memset( &dstRegion, 0, sizeof(dstRegion) );
		dstRegion.uiDataOffset = uiTotalBytes;
		dstRegion.uiDataLength = uiBytes;
		dstRegion.sOffsetX = (INT16)iNewOffsetX;
		dstRegion.sOffsetY = (INT16)iNewOffsetY;
		dstRegion.usWidth = (UINT16)uiNewWidth32;
		dstRegion.usHeight = (UINT16)uiNewHeight32;
	...
	for ( UINT16 i = 0; i < usCount; ++i )
	{
		std::vector<UINT8> srcRGBA;
		if ( !VHDUnpackETRLERegionRGBA( hImage, i, srcRGBA ) )
		{
			MemFree( pNewData );
			MemFree( pNewRegions );
			return FALSE;
		...
		const ETRLEObject &srcRegion = hImage->pETRLEObject[i];
		const ETRLEObject &dstRegion = newRegions[i];
		UINT8 *pDstBase = pNewData + dstRegion.uiDataOffset;
		...
		for ( UINT16 y = 0; y < dstRegion.usHeight; ++y )
		{
			...
			{
				const UINT16 srcX = (UINT16)( x / ubScale );
				const UINT8 *pSrcPixel = &srcRGBA[
					( ( (size_t)srcY * srcRegion.usWidth ) + srcX ) * 4 ];
				UINT8 *pDstPixel = pDstBase +
					( ( (UINT32)y * dstRegion.usWidth + x ) * 4 );
	...
	hImage->p32BPPData = (UINT32*)pNewData;
	hImage->pETRLEObject = pNewRegions;
	hImage->uiSizePixData = uiTotalBytes;
	hImage->usWidth = usMaxWidth;
	hImage->usHeight = usMaxHeight;
